...
class FastRCNNOutputs(object):
    ...
    def __init__(
        ...
        proposals,
        smooth_l1_beta, losstype, bnrl_hyper
    ):
        """
        Args:
    ...
    def losses(self):
        ...
        if self.losstype != 'bnrlloss':
            ...
            cls_loss = BNRL(self.bnrl_hyper)
            our_loss = cls_loss(self.pred_class_logits, self.gt_classes)
        else:
            cls_loss = BNRL(self.bnrl_hyper)
            loss = {
            "loss_cls": cls_loss(self.pred_class_logits, self.gt_classes),
            "loss_box_reg": self.smooth_l1_loss(),
        }
...
class BNRL(nn.Module):
    ...
    def forward(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        negative_p = torch.zeros_like(x) + log_p.exp() 
        negative_p[all_rows, y] = 0.
        log_1_p = torch.log(1 - negative_p + 1e-26)
        negative_term = - torch.sum(negative_p**self.epsilon *log_1_p* self.weight, dim = 1) 
        ...
        positive_term =  focal_term * ce 
        loss = self.alpha* positive_term + self.beta* negative_term
